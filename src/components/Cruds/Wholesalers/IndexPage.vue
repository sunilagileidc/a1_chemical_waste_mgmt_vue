    ...
    >
      <template v-slot:item="props">
        <tr class="vdatatable_tbody">
          ...
          <td>
            <v-btn
              @click="updateWholesalerStatus(props.item.id)"
...
export default {
  ...
  methods: {
    ...
    async updateWholesalerStatus(status_id) {
      const result = await this.showConfirmation(
        "Confirm",
        "Are you sure you want to update this wholesaler status ?"
      );

      if (!result) return;
      this.loader = true;
      this.$axios
        .post("update_wholesaler_status", {
          id: status_id,
        })
        .then((res) => {
