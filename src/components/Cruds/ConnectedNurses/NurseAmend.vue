          ...
          <v-form ref="form" v-model="valid">
            <div class="row">
              <div class="col-md-12 pl-6">
                ...
                <v-row class="mt-1 px-4"> -->
                  <v-col cols="12" lg="4" sm="4" md="4">
                    <v-tooltip :text="$t('reg_no')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field
                          v-bind="props"
                          variant="outlined"
                          density="compact"
                          v-model="profile_details.reg_no"
                          v-bind:label="$t('reg_no')"
                          maxlength="100"
                          :rules="fieldRules"
                          class="required_field"
                        ></v-text-field>
                  ...
                  <v-col cols="12" lg="4" sm="4" md="4">
                    <v-tooltip :text="$t('job_title')" location="bottom">
                      <template v-slot:activator="{ props }">
                        <v-text-field
                          v-bind="props"
                          variant="outlined"
                          density="compact"
                          v-model="profile_details.job_title"
                  ...
                  <v-col cols="12" lg="8" sm="8" md="8">
                    <v-autocomplete
                      v-model="profile_details.institution_ids"
                      :items="institutions"
                      item-title="name"
                      item-value="id"
                      label="Institutions"
...
export default {
  ...
  data: () => ({
    ...
    sel_lang: "en",
    // sel_institution: "",
    institutionsFieldRules: [
      v => (Array.isArray(v) && v.length > 0) || 'Please select at least one institution'
    ]
  }),
    ...
    "$route.query.slug": {
      ...
      handler() {
        if (this.$route.query.slug) {
          ...
          this.$axios
            .get("fetch_connected_nurses_byslug/" + this.$route.query.slug)
            .then((res) => {
              if (res.data.status === "S") {
                this.profile_details = res.data.user;

                // MULTIPLE institutions handling
                if (res.data.user.other_user_details?.length) {
                  const details = res.data.user.other_user_details;

                  this.profile_details.institution_ids = details.map(
                    (item) => item.institution_id,
                  );

                  this.profile_details.reg_no = details[0]?.reg_no || null;

                  this.profile_details.job_title =
                    details[0]?.job_title || null;
                }
    ...
    fetchRoles() {
      ...
      this.$axios
        ...
        .then((response) => {
          ...
          const nurseRole = this.role_array.find(
            (role) => role.rolename === "Nurse",
          );
          if (nurseRole) {
            this.profile_details.role_id = nurseRole.id;
          }
          this.role_array_view_profile = nurseRole ? [nurseRole] : [];
          // this.profile_details.institution_id =
          //   this.sel_institution.institution_id;
          this.loader = false;
        })
        .catch((err) => {
          this.loader = false;
    ...
    async fetchInstitutions() {
      try {
        ...
        if (res.data.status === "S") {
          ...
          this.institutions = rawData.map((item) => ({
            id: item.institution_id,
            name: item.institution_data?.name || "N/A",
            full_data: item, // optional (if you need later)
          }));
        }
      } catch (error) {
        this.institutions = [];
