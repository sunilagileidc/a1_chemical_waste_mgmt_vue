              ...
              <tr class="vdatatable_tbody">
                ...
                <td>
                  <span v-if="props.item.address"
                    >{{ props.item.address }}-<span v-if="props.item.post_code"
                      >-{{ props.item.post_code }}</span
                    ></span
                  >
                  <span v-else>{{ $t("not_appllicable") }}</span>
                </td>
                <td>
                  <span v-if="props.item.ordering_address"
                    >{{ props.item.ordering_address
                    }}<span v-if="props.item.ordering_post_code"
                      >-{{ props.item.ordering_post_code }}</span
                    ></span
                  >
                  <span v-else>{{ $t("not_appllicable") }}</span>
                </td>
                <td>
                  <span v-if="props.item.delivery_address"
                    >{{ props.item.delivery_address
                    }}<span v-if="props.item.delivery_post_code"
                      >-{{ props.item.delivery_post_code }}</span
                    ></span
                  >
                  <span v-else>{{ $t("not_appllicable") }}</span>
                </td>
                ...
                <td class="px-0 text-center">
                  ...
                  <v-menu
                    v-if="
                      props.item.institution_contacts &&
                      props.item.institution_contacts.length
                    "
                    location="bottom"
                    open-on-hover
                    offset="10"
                  >
                    <template v-slot:activator="{ props }">
                      <v-icon
                        v-bind="props"
                        class="mr-2 edit_btn icon_size theme-color"
                      >
                    ...
                    <v-card min-width="280">
                      ...
                      <v-list density="compact" class="py-1">
                        <v-list-item
                          v-for="contact in props.item.institution_contacts"
                          :key="contact.id"
                        >
                          <template #prepend>
  ...
  methods: {
    ...
    initialize() {
      this.$axios
        .get("institution")
        .then((res) => {
          this.pharmacies = res.data.institutions.filter(
            (item) => item.institution_type !== "Hospital"
          );
          this.initval = false;
          this.loader = false;
        })
